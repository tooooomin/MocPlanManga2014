...
//Rare exceptions to indicate a bug in the _mangaview code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_mangaview = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_mangaview]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.mangaview.module"
